...
sleep 0.1;
		...
		_uniqueName=format ["<t color=""#E4FB44"">" + ("Helmet Cam Feed: %1") + "</t>",name _x];
		_monitor addAction [_uniqueName, "call fnc_PROCESS_LIVE",[_x,_debug],0,false,true,"","_target distance _this < 3.5"];

			_uniqueName=format ["<t color=""#84EFF2"">" + ("Infrared Satellite Feed : %1") + "</t>",name _x];
...
fnc_RENDER = {
	...
	_monitor setObjectTextureglobal  [0,(format ["#(argb,256,256,1)r2t(%1,1.0)",_name])]; // FIX THIS  Blurry BULLSHIT
